...
* registro.php
*
* instrucción para editar base de datos utilizando las variables de llamada}.
*  
* 
...
http://190.111.246.33:8008/SIGSAO/registroactivar.php?user=".$activacion."\n
				"; 
...
***NO RESPONDA ESTE CORREO ELECTRONICO****
			...
			include('./includes/zz_maildata');//carga la variable $ZZMD
			...
			$mail->Host = $ZZMD['Host'];
			$mail->SMTPAuth = true;
			$mail->Username = $ZZMD['Username']; 
			$mail->Password = $ZZMD['Password'];
...
<body>
	...
	<?php 
	 ...
	 if (isset($_GET['registrado'])) { ?>
	  <h2>Gracias</h2> Su usuario ha sido registrado con éxito. Ahora puede <a href="login.php">ingresar aqui</a>";
	 <?php exit();
	  }
	?>
		...
		<form action="registro.php" method="post">
			...
			<div>
			<label for="cargo">Cargo<span>Ingrese su Cargo, Puesto o Rol dentro de dicha organización</span></label>
			<input name="cargo" type="text" value="<?php echo $posts['cargo'];?>"/>
			</div>	
			...
			<div>
			<label for="cargo"><span>Acepto que esta información sea utilizada para actividades participativas.</span></label>
			<input name="acepto1" type="checkbox" class="required" value="si"/>
			<span class="error"><?php echo $ERROR['acepto1'];?></span>
			</div>	

			<div>
			<label for="cargo"><span>Acepto la plena responsabilidad por todo el material que suba a esta plataforma.</span></label>
			...
			<div class="aclaracionsubmit">
				Toda la información registrada será utilizada exclusivamente en el marco de los Procecos Participativas Urbanos para el desarrollo del Municipio de San Antonio Oeste
			</div>			
